...
@section('content')
<div class="container">
    <p class="text-3xl font-semibold">Tambah Project </p>

    
<form class="mt-3" method="post" action="{{ route('tambahProject') }}" enctype="multipart/form-data">
    @csrf
    ...
    <div class="mb-6">
      ...
      <input type="text" id="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @error('sistem')
          is_invalid
      @enderror" placeholder="Sistem Informasi Kang Cimol" name="sistem" value="{{ old('sistem') }}">
      @error('sistem')
          <div class="feedback">
            {{ $message }}
          </div>
    ...
    <div class="mb-6">
        ...
        <textarea id="message" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 @error('keterangan')
            is_invalid
        @enderror" placeholder="masukan deskripsi singkat tentang sistem mu jgn panjang panjang yah" name="keterangan">{{ old('keterangan') }}</textarea>
        @error('keterangan')
            <div class="feedback">
                {{ $message }}
            </div>
    ...
    <div class="mb-6 mt-5">
        ...
        <input type="text" id="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @error('link')
            is_invalid
        @enderror" placeholder="blablabla.xipplg2.com" name="link">
        @error('link')
        <div class="feedback">
            {{ $message }}
        </div>
      ...
      <div class="mb-6 mt-5">
        ...
        <input type="text" id="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 @error('vidio')
            is_invalid
        @enderror" placeholder="https://youtu.be/urtvpt08EJY" name="vidio">
        @error('vidio')
        <div class="feedback">
            {{ $message }}
        </div>
